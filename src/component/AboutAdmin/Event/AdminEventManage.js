...
export function AdminEventManage() {
  ...
  function handleEditItem(item) {
    navigate(`/sadkljf$ewulihfw_mcnjcbvjaskanshcbjancasuhbj/editEvent/${item.event_id}`);
  }
  ...
  const { mutate: filterMutation } = useMutation({ mutationFn: fetchFilteredEvents })
  ...
  // 상품 삭제를 처리하는 뮤테이션
  const { mutate: deleteEventMutation } = useMutation({ mutationFn: fetchDeletedEvents })
    ...
    <div className={styles.main}>
      <div className={styles.container}>
              ...
              {eventData.map((item, index) => (
                <React.Fragment key={index}>
                  <tr className={styles.list}>
                    <td>{item.event_id}</td>
                    <td><img className={styles.thumnail} src={item.event_image} alt='이미지'></img></td>
                    ...
                    <td>
                      {new Date(item.event_endDate).toLocaleDateString()}
                    </td>
                    <td>
                      <button className='white_round_button' onClick={()=>handleEditItem(item.event_id)}>수정</button>
                    </td>
                    <td>
                      <button className='white_round_button' onClick={()=>handleDeleted(item)}>삭제</button>
                    </td>
                  </tr>
                </React.Fragment>
              ))
        ...
        <div className={styles.buttonContainer}>
          <button className='original_round_button' onClick={()=>navigate(`/sadkljf$ewulihfw_mcnjcbvjaskanshcbjancasuhbj/event/create`)}>생성</button>
        </div>
